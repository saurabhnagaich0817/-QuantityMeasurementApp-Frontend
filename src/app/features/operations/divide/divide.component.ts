...
@Component({
  ...
  template: `
    <div class="container">
      <div class="card">
        <h1> Division</h1>
        <p class="subtitle">Divide one quantity by another (dimensionless result)</p>
        ...
        <div class="row">
          <div class="col">
            <label> Dividend (Numerator)</label>
          ...
          <div class="col">
            <label> Divisor (Denominator)</label>
        ...
        <div class="warning" *ngIf="secondValue === 0"> Division by zero is not allowed</div>

        <button (click)="calculate()" [disabled]="isLoading || secondValue === 0" class="btn-calculate">
          {{ isLoading ? ' Calculating...' : ' Calculate' }}
        </button>

        <div class="result-area" *ngIf="result && !isLoading">
          <div class="result-box" *ngIf="!result.isError">
            <div class="result-label">✅ RESULT (Dimensionless)</div>
            <div class="result-value">
              {{ firstValue }} {{ firstUnit }} ÷ {{ secondValue }} {{ secondUnit }} = 
              <span class="answer">{{ result.result }}</span>
            </div>
  ...
  styles: [`
    ...
    .operator { font-size: 48px; font-weight: bold; color: #4caf50; }
    ...
    .warning { background: #fff3e0; color: #ff9800; padding: 12px; border-radius: 10px; margin: 20px 0; text-align: center; }
    ...
    .answer { font-size: 32px; font-weight: bold; color: #4caf50; background: white; padding: 8px 20px; border-radius: 40px; display: inline-block; }
    ...
    @media (max-width: 600px) { .card { padding: 24px; } .row { flex-direction: column; } .operator-box { justify-content: center; margin: 10px 0; } .answer { font-size: 24px; } }
  `]
})
export class DivideComponent {
  ...
  calculate() {
    ...
    this.quantityService.divide(request).subscribe({
      ...
      error: (err) => {
        this.errorMessage = err.error?.detail || err.message || 'Division failed';
        this.isLoading = false;
      }
    });
  }
